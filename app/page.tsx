...
type SystemInfo = {
  runtime?: { deploymentMode: 'local' | 'vps'; nodeName: string; nodeVersion: string; uptimeSeconds: number; publicUrl: string | null; dataDirectory: string; storage: { available: boolean; freeBytes: number | null; totalBytes: number | null }; managementAccess: string };
...
function controlPlaneUrl(path: string, method = 'GET') {
  if (!usesLocalAgentBridge()) return path;
  return `${localAgentOrigin}${agentPath(path, method)}`;
}

function apiFetch(path: string, init: RequestInit = {}) {
  if (!usesLocalAgentBridge()) return fetch(path, init);
  return fetch(controlPlaneUrl(path, init.method), {
    ...init,
    mode: 'cors',
...
export default function Home() {
  ...
  const refresh = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const [systemData, sitesData, stagingData, clientsData, blueprintsData, pluginLibraryData, agentsData, activityData] = await Promise.all([
        systemResponse.json().catch(() => ({ connected: false, error: 'Unable to read hosting status.' })),
      ...
      setError(!systemResponse.ok || !systemData.connected ? systemData.error || sitesData.error || 'The GeekHeros hosting service is offline.' : null);
    } catch {
      const message = usesLocalAgentBridge()
        ...
        : 'The GeekHeros service briefly lost its connection. Check the hosting service, then retry.';
  ...
  async function createSite(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setToast(`${result.site.name} is provisioning in GeekHeros.`); await refresh(true);
  ...
  async function createStaging(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setToast(`${result.staging.name} is cloning production into isolated site services.`); await refresh(true);
  ...
  async function manageStaging(site: Site, action: 'sync' | 'promote' | 'delete') {
    ...
    if (action === 'delete' && !window.confirm(`Delete ${site.name}, including its isolated site services, volumes and backups? Production will not be changed.`)) return;
  ...
  async function operate(site: Site, type: string, options: Record<string, unknown> = {}) {
    if (type === 'delete' && !window.confirm(`Delete ${site.name}, its services, storage and backups? This cannot be undone.`)) return false;
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      <button className="brand" onClick={() => navigate('Overview')}><span className="brand-mark">G</span><span>geekheros.com</span></button>
      <div className="workspace-switcher static-workspace"><span className="workspace-avatar">GH</span><span className="workspace-copy"><strong>Hosting workspace</strong><small>{system.connected ? 'Node connected' : 'Agent offline'}</small></span><span className={`connection-light ${system.connected ? 'online' : ''}`} /></div>
      ...
      <div className="sidebar-user"><span className="user-avatar">GH</span><span className="workspace-copy"><strong>Administrator</strong><small>geekheros.com</small></span></div>
    </aside>
    <section className="workspace">
      <header className="topbar"><label className="global-search"><Search aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search sites, staging, domains, clients, blueprints and agents" /></label><div className={`top-actions ${system.connected ? '' : 'offline-copy'}`}><span className="live-dot" />{system.connected ? `GeekHeros online` : 'GeekHeros service offline'}<button className="icon-button" onClick={() => void refresh()} aria-label="Refresh"><RefreshCw aria-hidden="true" /></button></div></header>
      {error && <div className="connection-banner"><span><CircleAlert aria-hidden="true" /></span><div><strong>Site management is unavailable</strong><p>{hostingText(error)}</p></div><button onClick={() => void refresh()}>Retry connection</button></div>}
...
function StagingView({ stagingSites, productionSites, busy, onCreate, onManage, onBackup, onLogin, onOpen }: { stagingSites: Site[]; productionSites: Site[]; busy: string | null; onCreate: () => void; onManage: (site: Site, action: 'sync' | 'promote' | 'delete') => Promise<void>; onBackup: (site: Site) => Promise<boolean>; onLogin: (site: Site) => Promise<void>; onOpen: (id: string) => void }) {
  ...
  return <><PageHeading eyebrow="WORDPRESS WORKFLOW" title="Staging" description="Safely test WordPress changes in isolated site environments before pushing them to production." actions={<button className="primary-button" onClick={onCreate}><Plus aria-hidden="true" />Create staging</button>} />
...
function ClientsView({ clients, sites, busy, onAdd, onEdit, onDelete, onOpenSite }: { clients: Client[]; sites: Site[]; busy: string | null; onAdd: () => void; onEdit: (client: Client) => void; onDelete: (client: Client) => void; onOpenSite: (id: string) => void }) {
  ...
  return <><PageHeading eyebrow="CLIENT OWNERSHIP" title="Clients" description="Group real WordPress and Lovable sites by client without duplicate records." actions={<button className="primary-button" onClick={onAdd}><Plus aria-hidden="true" />Add client</button>} />
    <div className="metric-grid compact-metrics"><MetricCard icon={Users} tone="blue" label="Clients" value={String(clients.length)} detail="Stored in the local control plane" /><MetricCard icon={Container} tone="green" label="Assigned sites" value={String(assigned)} detail={`${sites.length - assigned} currently unassigned`} /><MetricCard icon={CirclePlay} tone="violet" label="Client sites online" value={String(sites.filter((site) => site.clientId && site.status === 'Running').length)} detail="Live hosting state" /><MetricCard icon={Package} tone="amber" label="Client updates" value={String(sites.filter((site) => site.clientId).reduce((sum, site) => sum + site.updates, 0))} detail="Pending maintenance items" /></div>
...
function SiteWorkspace({ site, clients, backupHistoryRevision, busy, onBack, onOperate, onRestoreBackup, onSaveMetadata, onSaveBackupSchedule, onLogin }: { site: Site; clients: Client[]; backupHistoryRevision: string; busy: string | null; onBack: () => void; onOperate: (site: Site, type: string, options?: Record<string, unknown>) => Promise<boolean>; onRestoreBackup: (sourceSiteId: string, backupId: string, targetSiteId: string, restoreScope: 'all' | 'files' | 'database') => Promise<boolean>; onSaveMetadata: (site: Site, clientId: string | null, tags: string[]) => Promise<boolean>; onSaveBackupSchedule: (site: Site, mode: BackupMode, intervalHours: number) => Promise<boolean>; onLogin: (site: Site) => Promise<void> }) {
  ...
  return <div className="site-workspace"><div className="site-hero"><div className="site-hero-toolbar"><button className="back-button" onClick={onBack}><ArrowLeft aria-hidden="true" />{site.environment === 'staging' ? 'Staging' : 'All sites'}</button><span>{site.kind === 'lovable' ? 'Lovable application' : site.environment === 'staging' ? 'Isolated WordPress staging environment' : 'Managed WordPress site'}</span></div><div className="site-profile-band"><SiteThumbnail key={`${site.id}:${site.status}:${site.screenshot?.capturedAt || ''}`} site={site} /><div className="site-profile-content"><div className="site-profile-top"><div className="site-profile-heading"><h1>{site.name}</h1><a href={site.siteUrl} target="_blank" rel="noreferrer"><Globe2 aria-hidden="true" />{site.domain}<ExternalLink aria-hidden="true" /></a></div><div className="site-actions">{site.kind === 'wordpress' ? <button className="primary-button site-login-button" disabled={!canOperate || site.status !== 'Running'} onClick={() => void onLogin(site)}><LogIn aria-hidden="true" />One-click WP Admin</button> : site.lovableBuildUrl && <a className="primary-button link-button site-login-button" href={site.lovableBuildUrl} target="_blank" rel="noreferrer"><Sparkles aria-hidden="true" />Open in Lovable</a>}<details className="site-action-menu"><summary><ServerCog aria-hidden="true" />Actions<ChevronDown aria-hidden="true" /></summary><div>{site.status === 'Stopped' ? <button disabled={!canOperate} onClick={() => void run('start')}><Play aria-hidden="true" />Start site</button> : <button disabled={!canOperate} onClick={() => void run('stop')}><Square aria-hidden="true" />Stop site</button>}<button disabled={!canOperate} onClick={() => void run('restart')}><RotateCw aria-hidden="true" />Restart site</button><button disabled={!canOperate} onClick={() => void run('refresh')}><RefreshCw aria-hidden="true" />Refresh site state</button><a href={site.directUrl || site.siteUrl} target="_blank" rel="noreferrer"><ExternalLink aria-hidden="true" />Open frontend</a></div></details></div></div><div className="site-profile-bottom"><div className="site-profile-labels"><Status site={site} />{site.environment === 'staging' && <span className="profile-badge staging"><FlaskConical aria-hidden="true" />Staging</span>}<span className="profile-badge">{site.pod}</span>{site.blueprintName && <span className="profile-badge blueprint">{site.blueprintName}</span>}<span className="client-badge"><Users aria-hidden="true" />{clientName}</span>{site.tags.map((tag) => <span className="tag-badge" key={tag}>{tag}</span>)}</div><div className="runtime-badges">{site.kind === 'wordpress' ? <><span><strong>WP</strong>{site.wp}</span><span><strong>PHP</strong>{site.php}</span></> : <><span><strong>{site.sourceProvider === 'lovable' ? 'Source' : 'Git'}</strong>{site.sourceProvider === 'lovable' ? 'Lovable' : site.repositoryBranch || 'Auto'}</span><span><strong>Rev</strong>{shortRevision(site.sourceRevision)}</span></>}</div></div></div></div><nav className="site-tabs" aria-label="Site management">{tabs.map(({ id, label, icon: Icon }) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => setTab(id)}><Icon aria-hidden="true" />{label}{id === 'Updates' && site.updates > 0 && <em>{site.updates}</em>}</button>)}</nav></div>
    <div className="site-tab-content">{site.error && <div className="site-error"><span><CircleAlert aria-hidden="true" /></span><div><strong>Last operation failed</strong><p>{hostingText(site.error)}</p></div></div>}{site.phase && <div className="provisioning-banner"><span className="spinner" /><div><strong>{hostingText(site.phase)}</strong><p>GeekHeros is applying the requested hosting state. This page refreshes automatically.</p></div></div>}{site.kind === 'wordpress' && inventoryError && <div className="site-error"><span><CircleAlert aria-hidden="true" /></span><div><strong>Live WordPress inventory unavailable</strong><p>{inventoryError}</p></div></div>}{tab === 'Backups' && backupHistoryError && <div className="site-error"><span><CircleAlert aria-hidden="true" /></span><div><strong>Backup history unavailable</strong><p>{backupHistoryError}</p></div></div>}
...
const developerTools: Array<{ type: DeveloperToolType; title: string; copy: string; icon: LucideIcon; wordpressOnly?: boolean }> = [
  { type: 'terminal', title: 'Site terminal', copy: 'Audited command-line access scoped to the site service.', icon: Terminal },
  ...
  { type: 'logs', title: 'Log viewer', copy: 'Read timestamped web application and database logs.', icon: Logs },
  ...
  { type: 'runtime', title: 'Web server', copy: 'Inspect hosting services, routing and resource limits.', icon: ServerCog },
];
...
function ToolsPanel({ site, disabled, onRun, onLogin }: { site: Site; disabled: boolean; onRun: (type: string, options?: Record<string, unknown>) => Promise<void>; onLogin: () => void }) {
  ...
  return <><section className="content-card"><div className="card-title"><div><small>{isLovable ? 'LOVABLE & CONTAINER' : 'WORDPRESS & CONTAINER'}</small><h2>Site tools</h2></div></div><div className="operation-grid">{isLovable ? <><Operation icon={RefreshCw} title="Check source" copy={directSource ? 'Compare the connected Lovable project with the deployed revision.' : 'Compare the deployed commit with the configured Git branch.'} disabled={disabled} onClick={() => void onRun('refresh')} /><Operation icon={Upload} title="Deploy latest" copy={directSource ? 'Back up, import the latest Lovable source, rebuild and replace the application service.' : 'Back up the checkout, pull the branch, rebuild and replace the application service.'} disabled={disabled} onClick={() => void onRun('redeploy')} /><Operation icon={ArchiveRestore} title="Create backup" copy="Archive the exact imported source used for this build." disabled={disabled} onClick={() => void onRun('backup')} /><Operation icon={ShieldCheck} title="Verify source" copy={directSource ? 'Confirm the connected project and local package definition are readable.' : 'Confirm the repository branch and local package definition are readable.'} disabled={disabled} onClick={() => void onRun('scan')} /><Operation icon={RotateCw} title="Restart service" copy="Restart the application service without rebuilding." disabled={disabled} onClick={() => void onRun('restart')} />{site.lovableBuildUrl && <a className="operation-card" href={site.lovableBuildUrl} target="_blank" rel="noreferrer"><span><Sparkles aria-hidden="true" /></span><strong>Continue in Lovable</strong><small>Open the connected project for further generation.</small><em>Open Lovable<ExternalLink aria-hidden="true" /></em></a>}</> : <><Operation icon={RefreshCw} title="Refresh inventory" copy="Read live core, plugin, theme and PHP versions." disabled={disabled} onClick={() => void onRun('refresh')} /><Operation icon={Upload} title="Update everything" copy="Back up, then update WordPress core, all plugins and all themes." disabled={disabled} onClick={() => void onRun('update')} /><Operation icon={ArchiveRestore} title="Create backup" copy="Export MariaDB and archive the WordPress files." disabled={disabled} onClick={() => void onRun('backup')} /><Operation icon={ShieldCheck} title="Verify checksums" copy="Validate WordPress core and available plugin checksums." disabled={disabled} onClick={() => void onRun('scan')} /><Operation icon={RotateCw} title="Restart services" copy="Restart MariaDB and WordPress in dependency order." disabled={disabled} onClick={() => void onRun('restart')} /><Operation icon={LogIn} title="One-click WP Admin" copy="Issue a one-time, 60-second administrator session." disabled={disabled} onClick={onLogin} /></>}</div></section><section className="content-card developer-tools-card"><div className="card-title"><div><small>DEVELOPER ACCESS</small><h2>Developer tools</h2><p>Audited maintenance and troubleshooting tools for this site.</p></div></div><div className="tool-grid">{developerTools.map(({ type, title, copy, icon: Icon, wordpressOnly }) => <button className="tool-card" key={type} disabled={disabled || (wordpressOnly && isLovable)} onClick={() => setDeveloperTool(type)}><span><Icon aria-hidden="true" /></span><strong>{title}</strong><small>{wordpressOnly && isLovable ? 'Available for managed WordPress sites.' : copy}</small><em>Open tool<ArrowRight aria-hidden="true" /></em></button>)}</div></section><section className="content-card tool-details"><div className="card-title"><div><small>RUNTIME DETAILS</small><h2>Service details</h2></div></div><div className="definition-grid"><span><small>{isLovable ? 'Application' : 'WordPress'}</small><strong>{site.containerName}</strong></span><span><small>{isLovable ? directSource ? 'Source' : 'Source branch' : 'MariaDB'}</small><strong>{isLovable ? directSource ? 'Connected Lovable project' : site.repositoryBranch || 'Auto-detect' : site.databaseContainer}</strong></span><span><small>Runtime package</small><strong>{site.image}</strong></span><span><small>Service ID</small><strong>{site.containerId || '—'}</strong></span><span><small>Domain</small><strong>{site.siteUrl}</strong></span><span><small>Local preview</small><strong>{site.directUrl || '—'}</strong></span></div></section><section className="danger-zone"><div><strong>Remove site</strong><p>{isLovable ? 'Deletes the application service, build, source checkout and backups.' : 'Deletes the site services, files, database and backups.'}</p></div><button disabled={disabled} onClick={() => void onRun('delete', { deleteData: true })}>Delete site and data</button></section>{developerTool && <DeveloperToolModal site={site} tool={developerTool} onClose={() => setDeveloperTool(null)} />}</>;
}
...
function ActivityView({ entries }: { entries: ActivityEntry[] }) { return <><PageHeading eyebrow="AUDIT LOG" title="Activity" description="Completed and failed operations reported by the local GeekHeros service." /><section className="content-card"><ActivityList entries={entries} /></section></>; }
function ActivityList({ entries }: { entries: ActivityEntry[] }) { if (!entries.length) return <Empty title="No operations recorded" copy="Site launches and lifecycle actions will appear here." />; return <div className="timeline live-timeline">{entries.map((entry) => <div key={entry.id}><span className={`activity-dot ${entry.state === 'failed' ? 'red' : 'green'}`} /><span><strong>{hostingText(entry.message)}</strong><small>{entry.siteName} · {entry.type}</small></span><time>{relativeTime(entry.createdAt)}</time></div>)}</div>; }
...
function SettingsView({ system, onToast }: { system: SystemInfo; onToast: (message: string) => void }) {
  ...
  return <>
    <PageHeading eyebrow="GEEKHEROS.COM" title="Settings" description="Manage your hosting node, integrations and AI access." />
    <div className="settings-layout">
      <aside className="settings-nav">
        <button className="selected">Hosting connection</button>
        <a href="#hosting-readiness">Deployment readiness</a>
        <a href="#mcp-connection">AI access</a>
      </aside>
      <section className="settings-main">
        <div className="settings-section">
          <div><h2>Hosting node</h2><p>Live details reported by the node receiving control-plane operations.</p></div>
        ...
        {system.runtime && <div className="settings-section" id="hosting-readiness">
          <div><h2>Deployment readiness</h2><p>Live configuration and storage measurements from your GeekHeros node.</p></div>
          <div className="definition-grid">
            <span><small>Deployment</small><strong>{system.runtime.deploymentMode === 'vps' ? 'VPS service' : 'Local development'}</strong></span>
            <span><small>Node</small><strong>{system.runtime.nodeName}</strong></span>
            <span><small>Management access</small><strong>{system.runtime.managementAccess}</strong></span>
            <span><small>Service uptime</small><strong>{Math.floor(system.runtime.uptimeSeconds / 60)} minutes</strong></span>
            <span><small>Storage available</small><strong>{system.runtime.storage.freeBytes === null ? 'Unavailable' : formatBytes(system.runtime.storage.freeBytes)}</strong></span>
            <span><small>Management address</small><strong>{system.runtime.publicUrl || 'This computer'}</strong></span>
          </div>
          <div className="setup-note"><span><Info aria-hidden="true" /></span><p>{system.runtime.deploymentMode === 'vps' ? 'The service can run continuously without a desktop session. Keep a separate off-server copy of backups and control-plane data.' : 'This is your development node. A standalone VPS build and managed-service setup are included; moving production data requires a planned migration.'}</p></div>
        </div>}
        <div className="settings-section" id="mcp-connection">
          <div className="mcp-section-heading"><div><h2>AI agent MCP</h2><p>Connect an MCP-compatible agent to your control-plane capabilities.</p></div>{system.mcp && <span>{system.mcp.toolCount} tools</span>}</div>
          ...
          <div className="setup-note mcp-setup-note"><span><Info aria-hidden="true" /></span><p>{system.runtime?.deploymentMode === 'vps' ? 'Agents connect securely to your VPS over HTTPS while you are away. The embedded bearer token grants management access; share it only with trusted agents.' : 'This development endpoint is available to agents on this computer while the local service is running. Treat the embedded bearer token like an administrator password.'}</p></div>
        </div>
        <div className="settings-section">
          ...
          <div className="setup-note lovable-setup-note"><span><Info aria-hidden="true" /></span><p>The authorization token stays in the ignored local GeekHeros state file and is never sent to the dashboard browser or stored in service metadata.</p></div>
        </div>
        <div className="settings-section">
          ...
          <div className="setup-note"><span><Info aria-hidden="true" /></span><p>GitHub and GitLab are optional for direct Lovable launches. Lovable authorization is never exposed to build services.</p></div>
        </div>
        <div className="settings-section">
...
function LovableLaunchModal({ clients, busy, onBack, onClose, onOpenSettings, onSubmit }: { clients: Client[]; busy: boolean; onBack: () => void; onClose: () => void; onOpenSettings: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target && !locked) onClose(); }}>
    <section className="modal lovable-launch-modal" role="dialog" aria-modal="true" aria-labelledby="lovable-launch-title">
      <div className="modal-head"><div><p className="eyebrow">LOVABLE WORKLOAD</p><h2 id="lovable-launch-title">Build with Lovable</h2><p>Create the project through your linked Lovable account, then let GeekHeros import, build and host its source on geekheros.com.</p></div><button onClick={onClose} disabled={locked} aria-label="Close"><X aria-hidden="true" /></button></div>
      <form onSubmit={onSubmit}>
        <input type="hidden" name="kind" value="lovable" />
        <input type="hidden" name="sourceProvider" value="lovable" />
        <div className="lovable-workflow"><span>1</span><p><strong>Generate</strong>Create the project through the linked Lovable account.</p><span>2</span><p><strong>Import</strong>Read the generated source directly from Lovable.</p><span>3</span><p><strong>Host</strong>Build the project and launch its site service.</p></div>
        <div className="form-grid">
          ...
          <label>Hosting plan<select name="pod" defaultValue="Standard"><option>Micro</option><option>Standard</option><option>Performance</option><option>Power</option></select></label>
...
function hostingText(text: string) { return text.replace(/Docker Desktop/gi, 'GeekHeros hosting').replace(/\bDocker\b/gi, 'hosting').replace(/\bcontainers\b/gi, 'services').replace(/\bcontainer\b/gi, 'service').replace(/\bvolumes\b/gi, 'storage'); }
function messageFrom(value: unknown, fallback: string) { return hostingText(value instanceof Error ? value.message : fallback); }
