  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        <button className="brand" onClick={() => navigate('Overview')}><span className="brand-mark">S</span><span>StreamHost</span></button>
        <button className="workspace-switcher">
          <span className="workspace-avatar">SH</span><span className="workspace-copy"><strong>StreamHost Pro</strong><small>Business workspace</small></span><span className="chevron">⌄</span>
        </button>
...
function SettingsView({ onToast }: { onToast: (message: string) => void }) {
  ...
  return <><PageHeading eyebrow="WORKSPACE" title="Settings" description="Infrastructure defaults, notifications and team access." actions={<button className="primary-button" onClick={() => onToast('Workspace settings saved.')}><span>✓</span>Save changes</button>} /><div className="settings-layout"><aside className="settings-nav"><button className="selected">Workspace</button><button>Deployment defaults</button><button>Backup policy</button><button>Notifications</button><button>Team & access</button><button>API tokens</button></aside><section className="settings-main"><div className="settings-section"><div><h2>Workspace profile</h2><p>Used throughout reports and operational notifications.</p></div><div className="form-grid"><label>Workspace name<input defaultValue="StreamHost Pro" /></label><label>Business URL<input defaultValue="https://streamhostpro.net" /></label><label className="full-field">Default deployment domain<input defaultValue="sites.streamhostpro.net" /></label></div></div><div className="settings-section"><div><h2>Operational defaults</h2><p>Applied to newly deployed sites.</p></div><div className="setting-toggle"><span><strong>Encrypted scheduled backups</strong><small>Daily backup to configured S3-compatible storage.</small></span><button className={`switch ${backups ? 'on' : ''}`} onClick={() => setBackups(!backups)}><i /></button></div><div className="setting-toggle"><span><strong>Automated security scans</strong><small>Check inventory and file integrity every 12 hours.</small></span><button className={`switch ${security ? 'on' : ''}`} onClick={() => setSecurity(!security)}><i /></button></div><div className="setting-toggle"><span><strong>Slack operation alerts</strong><small>Send failures and recoveries to the operations channel.</small></span><button className={`switch ${slack ? 'on' : ''}`} onClick={() => setSlack(!slack)}><i /></button></div></div></section></div></>;
}
