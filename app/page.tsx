...
const nav: Array<{ view: View; symbol: string }> = [
  { view: 'Overview', symbol: '⌂' }, { view: 'Sites', symbol: '▦' }, { view: 'Clients', symbol: '♙' },
...
export default function Home() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Primary navigation"><p className="nav-label">Control plane</p>{nav.map((item) => <button key={item.view} onClick={() => navigate(item.view)} className={`nav-item ${view === item.view && !selected ? 'active' : ''}`}><span>{item.symbol}</span>{item.view}{item.view === 'Sites' && <em>{sites.length}</em>}{item.view === 'Clients' && <em>{clients.length}</em>}</button>)}</nav>
    ...
    <section className="workspace">
      <header className="topbar"><label className="global-search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search sites, clients and tags" /></label><div className={`top-actions ${system.connected ? '' : 'offline-copy'}`}><span className="live-dot" />{system.connected ? `Docker ${system.dockerVersion}` : 'Docker agent offline'}<button className="icon-button" onClick={() => void refresh()} aria-label="Refresh">↻</button></div></header>
...
function Overview({ sites, clients, system, activity, onLaunch, onOpen }: { sites: Site[]; clients: Client[]; system: SystemInfo; activity: ActivityEntry[]; onLaunch: () => void; onOpen: (id: string) => void }) {
  ...
  return <><PageHeading eyebrow="LOCAL INFRASTRUCTURE" title="Docker fleet" description="Live sites, client ownership and maintenance state from Docker Desktop." actions={<button className="primary-button" onClick={onLaunch}><span>＋</span>Launch WordPress</button>} />
    <div className="metric-grid"><MetricCard icon="●" tone="green" label="Running sites" value={String(system.runningSites || 0)} detail={`${system.managedSites || 0} managed site${system.managedSites === 1 ? '' : 's'}`} /><MetricCard icon="♙" tone="blue" label="Clients" value={String(clients.length)} detail={`${unassigned} unassigned site${unassigned === 1 ? '' : 's'}`} /><MetricCard icon="↻" tone="amber" label="Updates" value={String(sites.reduce((sum, site) => sum + site.updates, 0))} detail={`${stopped} stopped site${stopped === 1 ? '' : 's'}`} /><MetricCard icon="▤" tone="violet" label="Docker containers" value={String(system.runningContainers || 0)} detail={`${system.totalContainers || 0} total on this engine`} /></div>
...
function SiteWorkspace({ site, clients, busy, onBack, onOperate, onSaveMetadata, onLogin }: { site: Site; clients: Client[]; busy: string | null; onBack: () => void; onOperate: (site: Site, type: string, options?: Record<string, unknown>) => Promise<boolean>; onSaveMetadata: (site: Site, clientId: string | null, tags: string[]) => Promise<boolean>; onLogin: (site: Site) => Promise<void> }) {
  ...
  return <div className="site-workspace"><div className="site-hero"><button className="back-button" onClick={onBack}>← All sites</button><div className="site-hero-row"><div className="site-identity"><span className="large-site-avatar">{initials(site.name)}</span><div><div className="identity-title"><h1>{site.name}</h1><Status site={site} /></div><a href={site.siteUrl} target="_blank" rel="noreferrer">{site.domain} ↗</a><div className="hero-tags">{site.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></div></div><div className="site-actions">{site.status === 'Stopped' ? <button className="secondary-button" disabled={!canOperate} onClick={() => void run('start')}><span>▶</span>Start</button> : <button className="secondary-button" disabled={!canOperate} onClick={() => void run('stop')}>Stop</button>}<button className="secondary-button" disabled={!canOperate} onClick={() => void run('restart')}>Restart</button><button className="primary-button" disabled={!canOperate || site.status !== 'Running'} onClick={() => void onLogin(site)}>One-click WP Admin ↗</button></div></div><div className="site-quick-meta"><span><small>WordPress</small><strong>{site.wp}</strong></span><span><small>PHP</small><strong>{site.php}</strong></span><span><small>Profile</small><strong>{site.pod}</strong></span><span><small>Container</small><strong>{site.containerId || 'Preparing'}</strong></span><span><small>Uptime</small><strong>{site.uptime}</strong></span></div><nav className="site-tabs">{(['Overview', 'Updates', 'Backups', 'Tools'] as SiteTab[]).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item}{item === 'Updates' && site.updates > 0 && <em>{site.updates}</em>}</button>)}</nav></div>
    <div className="site-tab-content">{site.error && <div className="site-error"><span>!</span><div><strong>Last operation failed</strong><p>{site.error}</p></div></div>}{site.phase && <div className="provisioning-banner"><span className="spinner" /><div><strong>{site.phase}</strong><p>GeekHeros is applying the requested Docker state. This page refreshes automatically.</p></div></div>}{inventoryError && <div className="site-error"><span>!</span><div><strong>Live WordPress inventory unavailable</strong><p>{inventoryError}</p></div></div>}
...
function ToolsPanel({ site, disabled, onRun, onLogin }: { site: Site; disabled: boolean; onRun: (type: string, options?: Record<string, unknown>) => Promise<void>; onLogin: () => void }) {
  return <><section className="content-card"><div className="card-title"><div><small>WORDPRESS & CONTAINER</small><h2>Site tools</h2></div></div><div className="operation-grid"><Operation icon="↻" title="Refresh inventory" copy="Read live core, plugin, theme and PHP versions." disabled={disabled} onClick={() => void onRun('refresh')} /><Operation icon="↑" title="Update everything" copy="Back up, then update WordPress core, all plugins and all themes." disabled={disabled} onClick={() => void onRun('update')} /><Operation icon="◫" title="Create backup" copy="Export MariaDB and archive the WordPress volume." disabled={disabled} onClick={() => void onRun('backup')} /><Operation icon="♢" title="Verify checksums" copy="Validate WordPress core and available plugin checksums." disabled={disabled} onClick={() => void onRun('scan')} /><Operation icon="↯" title="Restart containers" copy="Restart MariaDB and WordPress in dependency order." disabled={disabled} onClick={() => void onRun('restart')} /><Operation icon="→" title="One-click WP Admin" copy="Issue a one-time, 60-second administrator session." disabled={disabled} onClick={onLogin} /></div></section><section className="content-card tool-details"><div className="card-title"><div><small>RUNTIME DETAILS</small><h2>Container endpoints</h2></div></div><div className="definition-grid"><span><small>WordPress</small><strong>{site.containerName}</strong></span><span><small>MariaDB</small><strong>{site.databaseContainer}</strong></span><span><small>Image</small><strong>{site.image}</strong></span><span><small>Container ID</small><strong>{site.containerId || '—'}</strong></span><span><small>Domain</small><strong>{site.siteUrl}</strong></span><span><small>Local preview</small><strong>{site.directUrl || '—'}</strong></span></div></section><section className="danger-zone"><div><strong>Remove site</strong><p>Deletes both containers, their named volumes and local backups.</p></div><button disabled={disabled} onClick={() => void onRun('delete', { deleteData: true })}>Delete site and data</button></section></>;
}
