...
export async function agentFetch(path: string, init: RequestInit = {}) {
  ...
  if (!response.ok) {
    const payload = await response.clone().json().catch(() => ({ error: 'The Docker agent returned an unreadable response.' })) as Record<string, unknown>;
    throw Object.assign(new Error(String(payload.error || 'Docker agent request failed.')), { status: response.status });
  }
  return response;
}
